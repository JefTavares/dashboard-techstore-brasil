...
def g1_receita_lucro(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            fill="tozeroy",
            fillcolor="rgba(77,159,255,0.10)",
...
def g6_margem_tempo(df: pd.DataFrame) -> go.Figure:
    m = (
        df.groupby("MesTS")
        .apply(
            lambda x: x["Lucro"].sum() / x["Valor_Total"].sum() * 100,
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        periodo = st.radio(
            "Período de análise", ["6 meses", "12 meses", "Tudo"], index=2
        )
        st.markdown(
    ...
    with c1:
        ...
        st.markdown(
            f'<div class="sec-title">Evolução Mensal — Receita vs. Lucro</div>'
    ...
    with c2:
        top_cat = (
            df.groupby("Categoria")["Valor_Total"].sum().idxmax() if len(df) else "—"
        )
        st.markdown(
            f'<div class="sec-title">Receita por Categoria</div>'
    ...
    with c3:
        top_canal = (
            df.groupby("Canal_Venda")["Valor_Total"].sum().idxmax() if len(df) else "—"
        )
        st.markdown(
            f'<div class="sec-title">Receita por Canal de Venda</div>'
    ...
    with c4:
        ...
        st.markdown(
            f'<div class="sec-title">Receita por Região</div>'
    ...
    # ── G5 full width ─────────────────────────────────────────────────────────
    st.markdown(
        '<div class="sec-title">Top 10 Produtos — Receita Total</div>'
    ...
    with c5:
        margem_media = (
            df["Lucro"].sum() / df["Valor_Total"].sum() * 100 if len(df) else 0
        )
        st.markdown(
            f'<div class="sec-title">Evolução da Margem de Lucro</div>'
    ...
    with c6:
        st.markdown(
            '<div class="sec-title">Vendas por Forma de Pagamento</div>'
    ...
    # ── G8 ────────────────────────────────────────────────────────────────────
    st.markdown(
        '<div class="sec-title">Ranking de Vendedores — Top 5</div>'
    ...
    # ── Monthly table ─────────────────────────────────────────────────────────
    st.markdown(
        '<div class="sec-title">Resumo Mensal Detalhado</div>'
